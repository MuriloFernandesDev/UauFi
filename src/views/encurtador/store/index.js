...
export const getEncurtador = createAsyncThunk(
  "encurtador/getEncurtador",
  async (parametros) => {
    //console.log(parametros)
    const response = await api.get("/url_encurtada/lista/", {
      params: parametros,
    })
...
export const deleteEncurtador = createAsyncThunk(
  "encurtador/deleteEncurtador",
  async (id, { dispatch, getState }) => {
    await api.delete(`/url_encurtada/${id}`)
    await dispatch(getEncurtador(getState().url_encurtada.params))
    return id
  }
)

export const EncurtadorSlice = createSlice({
  name: "encurtador",
  initialState: {
    data: [],
    total: -1,
  ...
  extraReducers: (builder) => {
    builder.addCase(getEncurtador.fulfilled, (state, action) => {
...
export default EncurtadorSlice.reducer
