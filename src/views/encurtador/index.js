...
// ** Store & Actions
import { getEncurtador, deleteEncurtador } from './store'
      ...
      <Row>
        <Col lg="6" className="d-flex align-items-center px-0 px-lg-1">
          ...
          <Button
            ...
            to="/encurtador/add"
            color="primary"
            disabled={!permissao.can('create', 'encurtador_url')}
          >
            {t('Nova url encurtada')}
          </Button>
        </Col>
        <Col
          lg="6"
...
const EncurtadorList = () => {
  // ** Store vars
  const dispatch = useDispatch()
  const store = useSelector((state) => state.url_encurtada)
  ...
  const handleDeleteConfirmation = (row) => {
    return MySwal.fire({
      ...
      if (result.value) {
        await dispatch(deleteEncurtador(row.id))
            ...
            <div className="d-flex flex-column">
              <Link to={`/encurtador/${row.id}`} id={`pw-tooltip2-${row.id}`}>
                ...
                <small className="text-truncate text-muted mb-0">
                  {row.cliques} Clique{row.cliques !== 1 ? 's' : ''}
                </small>
              </Link>
              <div className="text-truncate text-muted mb-0">{row.hash}</div>
            </div>
          </div>
        )
      },
          ...
          <div className="d-flex justify-content-left align-items-center">
            <div className="d-flex flex-column">
              <Link to={`/encurtador/${row.id}`} id={`pw-tooltip2-${row.id}`}>
                ...
                <small className="text-truncate text-muted mb-0">
                  {t('Cadastrado em')}: {formatDateTime(row.data_cadastro)}
                </small>
              </Link>
            </div>
          </div>
