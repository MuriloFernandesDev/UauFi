    ...
    <div className="w-100 py-2">
      <Row>
        <Col lg="6" className="d-flex align-items-center px-0 px-lg-1">
          ...
          <Button
            tag={Link}
            to="/pesquisa_captive/add"
            color="primary"
            disabled={!permissao.can('create', 'adm_pesquisa')}
          >
            {t('Nova pesquisa')}
          </Button>
        </Col>
...
const PesquisaCaptiveList = () => {
  // ** Store vars
  const dispatch = useDispatch()
  const store = useSelector((state) => state.pesquisa_captive)
  ...
  const handleDeleteConfirmation = (row) => {
    return MySwal.fire({
      ...
      if (result.value) {
        api
          .delete(`/pesquisa_captive/${row.id}`)
          .then((response) => {
        ...
        return (
          <div className="d-flex justify-content-left align-items-center">
            <div className="d-flex flex-column">
              <Link to={`/pesquisa_captive/${row.id}`}>
            ...
            <div className="d-flex flex-column">
              <Link to={`/pesquisa_captive/${row.id}`}>
                ...
                <small className="text-truncate text-muted mb-0">
                  {row.ativo ? (
                    <Badge color="success">{t('Ativa')}</Badge>
                  ) : null}{' '}
                  {row.qtd_respostas ?? 0} {t('respostas')}
                </small>
              </Link>
            </div>
          </div>
...
export default PesquisaCaptiveList
