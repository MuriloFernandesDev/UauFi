...
const checkForVisibleChild = (arr, ability) => {
  return arr.some((i) => {
    if (i.children) {
      ...
      return !i.action || ability.can(i.action, i.resource)
    }
  })
}
...
export const canViewMenuGroup = (item) => {
  ...
  return (
    (!item.action || ability.can(item.action, item.resource)) &&
    hasAnyVisibleChild
  )
}
...
export const canViewMenuItem = (item) => {
  const ability = useContext(AbilityContext)
  return !item.action || ability.can(item.action, item.resource)
}
